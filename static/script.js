...
function addGratitude() {
    ...
    .catch(() => showNotification('Error adding gratitude', 'error'));
}
...
function loadGratitude() {
    ...
    fetch('/gratitude')
        .then(res => res.text())
        .then(data => {
            document.getElementById('gratitude-list').innerHTML = data || 'No gratitude entries yet.';
            updateGratitudeCount();
        })
        .catch(() => document.getElementById('gratitude-list').innerHTML = 'Error loading gratitude entries.');
}
...
function updateGratitudeCount() {
    const entries = document.getElementById('gratitude-list').children.length;
    document.getElementById('gratitude-count').textContent = `Entries: ${Math.max(0, entries - 1)}`;
}
...
function sendChat(event) {
    if (event.key === "Enter") {
        ...
        fetch("/chat?msg=" + encodeURIComponent(msg))
            ...
            .then(reply => {
                document.getElementById("chat-log").innerHTML += `<div><b>MindBot:</b> ${reply}</div>`;
                chatLog.scrollTop = chatLog.scrollHeight;
            })
            .catch(() => {
                document.getElementById("chat-log").innerHTML += `<div><b>MindBot:</b> ⚠️ Sorry, I'm having trouble responding right now.</div>`;
            })
            .finally(() => {
                loading.style.display = "none";
            });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    showNotification('Welcome to MindScape!! Your mental wellness companion.', 'info');
});
